...
def generate_circle_polygon(
    lat: float, lon: float, radius_m: float, num_points: int = 64
):
    """
    ...
    project_to_latlon = pyproj.Transformer.from_proj(
    ...
    geojson_polygon = geojson.Feature(
        geometry=geojson.Polygon([list(circle_latlon.exterior.coords)])
    )
